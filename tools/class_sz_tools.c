...
// B.H. new (need to think how to do properly)

r = 4.*_PI_*pow(rs,3.)*get_gas_profile_at_x_M_c_z_break_b16_200c(x,
                                               V->m,
					       V->c,
                                               V->z,
                                               V->ptsz->A_rho0,
...
if (l<ptsz->array_profile_ln_k[0])
    return 0.;//l = ptsz->array_profile_ln_k[0];
 if (l>ptsz->array_profile_ln_k[ptsz->n_k_density_profile-1])
...
double get_normalization_gas_density_profile(double z_asked, double m_asked, struct tszspectrum * ptsz){
  ...
  if (z<ptsz->array_profile_ln_1pz[0]){
    ...
    result = exp(pwl_interp_2d(ptsz->n_z_density_profile,
                          ...
                          ptsz->array_profile_ln_1pz,
                          ptsz->array_profile_ln_m,
                          ptsz->array_profile_ln_rho_at_lnk_lnM_z[0],
                          1,
                          &z,
                          &m))/exp(m);
  }
